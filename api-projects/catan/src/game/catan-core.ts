...
export interface ITradingService {
  /**
   * Execute a maritime trade with the bank (e.g., 4:1).
   * Returns true if the trade succeeds and mutates player/bank resources.
   */
  tradeWithBank(
    player: PlayerState,
    bank: ResourceBundle,
  ...
  /** Validate if the given bundle is available in the player's hand at the requested quantities. */
...
export class DiceApiRandomService implements IRandomService {
  ...
  async rollDice(): Promise<{
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(`Dice API HTTP ${res.status}`);
      }
...
function addBundles(firstBundle: ResourceBundle, secondBundle: ResourceBundle) {
  for (const key of Object.values(ResourceType)) {
    if (key === ResourceType.Desert) {
      continue;
    }
    firstBundle[key] = (firstBundle[key] ?? 0) + (secondBundle[key] ?? 0);
  }
}

function subBundles(firstBundle: ResourceBundle, secondBundle: ResourceBundle) {
  for (const key of Object.values(ResourceType)) {
    if (key === ResourceType.Desert) {
      continue;
    }
...
function bundleCount(bundle: ResourceBundle): number {
  return Object.entries(bundle)
    .filter(([key]) => key !== ResourceType.Desert)
...
export class CatanEngine {
  ...
  addPlayer(id: string, name: string) {
    if (this.players.has(id)) {
      throw new Error('Player id exists.');
    }
  ...
  get currentPlayer(): PlayerState | null {
    if (this.currentPlayerOrder.length === 0) {
      return null;
    }
  ...
  private handleRobberSeven() {
    // Any player with 8+ cards must discard half (rounded down).
...
// ---------- Minimal "board" factory (placeholder geometry) ----------
function createStandardLikeTileSet(): Tile[] {
  // 19 tiles in base Catan, exactly one Desert with no number token.
  // Here we just provide a representative set and the usual number distribution (without geometry).
